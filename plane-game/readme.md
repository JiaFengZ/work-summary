...
- 游戏标题：设计游戏
- 游戏描述：这是一个射击游戏，使用 ← 和 → 操作你的飞机，使用空格（space）进行射击。
...
游戏中**飞机元素**是我们需要操作的主人公，以下是飞机元素的设计：
...
通过点击空格键，飞机将射击出**子弹元素**，飞机可同时射出多个子弹。以下是子弹元素的设计：
...
游戏中怪兽元素共有两种状态（存活和死亡），存活时怪兽会移动，死亡时会绽放成烟花。以下是怪兽元素的设定：
...
- 当子弹元素和怪兽元素发生碰撞时，怪兽元素处于死亡状态，且怪兽元素需绘制为爆炸图像 `img/boom.png` （爆炸过程绘制三帧，即怪兽死亡过程持续3帧）
- 怪兽死亡过程结束后，删除怪兽元素
...
## 多关卡设计
如下：
- 增加游戏关卡，实现不同的关卡有不同的难度（每一关相对上一关增加一行怪兽，怪兽移动速度增倍）
- 游戏可通过修改配置，来修改游戏
